...
pool = ConnectionPool(host='kika-backend-test1.intranet.com', port=6379, db=3, password='',decode_responses=True)
...
languages = ['ar','en_us','ru','es_us','ms_my']
...
def getState(userid):
    if userid not in user_state:
        lm_output_state = np.zeros([2, 2, 1, 200], dtype=np.float32)
        kc_output_state = np.zeros([2, 2, 1, 200], dtype=np.float32)
