...
languages = ['ar','en_us','ms_my','ru','es_us']
stopword={'en_us':['.',',','!','?','#','!!!','!','..','...','??',':','"','...'],'ar':['.'],'es_us':['.'],'ru':['.'],'ms_my':['.']}
unigram_trees={}
bigram_dicts={}
lstm_models = {}
#d1=redis.
...
def predict():
    ...
    if len(input_letters)!=0:
        ...
        #         break
    correction=result[0]
    return json.dumps({"status":"success","correction":correction,"candidates":result})
...
def readSql2dict(conn,d1):
    ...
    for data in results:
        ...
        sql2="select ar from user_info where user_id=(%s)"
        values = (data['user_id'])
        ...
        sql3="select ru from user_info where user_id=(%s)"
        values = (data['user_id'])
        ...
        sql4="select es_us from user_info where user_id=(%s)"
        values = (data['user_id'])
        ...
        sql5="select ms_my from user_info where user_id=(%s)"
        values = (data['user_id'])
        ...
        for j in y:
            if j=='en_us':
                ...
                try:
                    info_dict=d2[j]
                    a=str(info_dict)
                    sql2 = 'update user_info set ar ="%s" where user_id =(%s)'
                    cursor.execute(sql2 % (a,i))
                except Exception as e:print("插入失败",e)
            elif j=='es_us':
                try:
                    info_dict=d2[j]
                    a=str(info_dict)
                    sql2 = 'update user_info set es_us="%s" where user_id =(%s)'
                    cursor.execute(sql2 % (a,i))
                except Exception as e:print("插入失败",e)
            elif j=='ms_my':
                try:
                    info_dict=d2[j]
                    a=str(info_dict)
                    sql2 = 'update user_info set ms_my="%s" where user_id =(%s)'
                    cursor.execute(sql2 % (a,i))
                except Exception as e:print("插入失败",e)
            elif j=='ru':
                try:
                    info_dict=d2[j]
                    a=str(info_dict)
                    sql2 = 'update user_info set ru="%s" where user_id =(%s)'
                    cursor.execute(sql2 % (a,i))
                except Exception as e:print("插入失败",e)
    conn.commit()
